...
public class JSONRPCBridge implements Serializable{
	private static final long serialVersionUID = 7559422357440497837L;
	...
	if(debug)
	    LfwLogger.info("registered reference " + clazz.getName());
    }
	...
	if(debug)
	    LfwLogger.info("registered callable reference " + clazz.getName());
    }
	...
	if(debug)
	    LfwLogger.info("registered class " + clazz.getName() + " as " + name);
    }
	...
	synchronized (classMap) {
		...
		if(debug)
		    LfwLogger.info("unregistered class " + clazz.getName() +
			     " from " + name);
	    }
	}
    }
    ...
    protected ClassData resolveClass(String className)
    {
	Class clazz = null;
	ClassData cd = null;
	...
	if (cd != null) {
	    if(debug) LfwLogger.info("found class " + cd.clazz.getName() +
			       " named " + className);
	    return cd;
	}
	...
	if(debug)
	    LfwLogger.info("registered object " + o.hashCode() +
	...
	public void registerObject(Object key, Object o, Class interfaceClass)
	{
    	...
    	if(debug) {
    		LfwLogger.info("registered object " + o.hashCode() + " of class " + interfaceClass.getName() + " as " + key);
    	}
    }    
	...
	synchronized (objectMap) {
		...
		if(debug)
		    LfwLogger.info("unregistered object " + oi.o.hashCode() +
	...
	if(debug && oi != null)
	    LfwLogger.info("found object " + oi.o.hashCode() +
	...
	if(debug)
	    LfwLogger.info("registered callback " + callback.getClass().getName() +
	...
	if(debug)
	    LfwLogger.info("unregistered callback " + callback.getClass().getName() +
	...
	LfwLogger.info("registered local arg resolver " +
	...
	synchronized (localArgResolverMap) {
	    ...
	    if(resolverSet == null ||
	       !resolverSet.remove(new LocalArgResolverData(argResolver,
							    ...
							    contextInterface)))
		{
		    LfwLogger.error("local arg resolver " +
				argResolver.getClass().getName() +
	...
	LfwLogger.info("unregistered local arg resolver " +
		 argResolver.getClass().getName() +
		 " for local class " + argClazz.getName() +
		 " with context " + contextInterface.getName());
    }
	...
	if(o instanceof Method) {
	    Method m = (Method)o;
	    if(debug)
		LfwLogger.info("found method " + methodName +
			 "(" + argSignature(m) + ")");
	    return m;
	}
	...
	if(debug)
	    LfwLogger.info("looking for method " + methodName +
		     "(" + argSignature(arguments) + ")");
	for(int i=0; i < method.length; i++) {
	    try {
		candidate.add(tryUnmarshallArgs(method[i], arguments));
		if(debug)
		    LfwLogger.info("+++ possible match with method " + methodName +
			     "(" + argSignature(method[i]) + ")");
	    } catch (Exception e) {
		if(debug)
		    LfwLogger.info("xxx " + e.getMessage() + " in " + methodName +
			     "(" + argSignature(method[i]) + ")");
	    }
	}
	...
	try {
	    ...
	    LfwLogger.error("no method or parameters in request");
	    return new JSONRPCResult(JSONRPCResult.CODE_ERR_NOMETHOD, null,
				     JSONRPCResult.MSG_ERR_NOMETHOD);
	}

	if(isDebug())
	    LfwLogger.info("call " + encodedMethod + "(" + arguments + ")" +
		     ", requestId=" + requestId);
	...
	String callmethod = "json:" + className + ":" + methodName;
	ThreadTracer.getInstance().setRemoteCallMethod(callmethod);
	...
	try {
	    if(debug)
		LfwLogger.info("invoking " + method.getReturnType().getName() +
			 " " + method.getName() +
			 "(" + argSignature(method) + ")");
