...
function SpliterComp(parent, name, top, left, width, height, position, prop, orientation, oneTouch, attrAry, className) {
	...
	if (attrAry != null) {
		...
		this.isRunMode = getBoolean(attrAry.isRunMode, this.isRunMode);
	}
...
SpliterComp.prototype.manageSelf = function() {
	...
	//不显示拖动条
		...
		if(this.isRunMode){
			...
			this.divFeel.style.display = "none";
			this.divFeel.style.zIndex = getTopZIndex(this.divFeel.style.zIndex);
			this.Div_gen.appendChild(this.divFeel);
			this.divBtn = $ce("DIV");
			this.divBtn.id = "spliterDivBtn";
			this.divBtn.className = "spliter_btn";
			this.divBtn.style.display = "none";
			this.divBtn.style.zIndex = this.divFeel.style.zIndex + 1;
			this.Div_gen.appendChild(this.divBtn);
			
			this.divFeel.onmouseout = function(e){
				$ge("spliterDivBtn").placeIn = "0";
				if(this.timeoutID){
					window.clearTimeout(this.timeoutID);
				}
				this.timeoutID = window.setTimeout(function(){
					if($ge("spliterDivBtn").placeIn == "0"){
						$ge("spliterDivBtn").style.display = "none";
					}
				},3000);
				// 删除事件对象（用于清除依赖关系）
				clearEventSimply(e);
			...
			this.divFeel.onmouseover = function(e){
				$ge("spliterDivBtn").placeIn = "1";
				$ge("spliterDivBtn").style.display = "";
				blink();
				if(this.timeoutID){
					window.clearTimeout(this.timeoutID);
				}
			...
			this.divBtn.onmouseover = function(e){
				$ge("spliterDivBtn").placeIn = "1";
				$ge("spliterDivBtn").style.display = "";
			...
			this.divBtn.onmouseout = function(e){
				blink();
				$ge("spliterFeelDiv").onmouseout(e);
				// 删除事件对象（用于清除依赖关系）
				clearEventSimply(e);
			}
			
			this.divBtn.onclick = function(e){
				$ge("spliterDivBtn").placeIn = "0";
				$ge("spliterDivBtn").style.display = "none";
				$ge("spliterFeelDiv").style.display = "none";
				$ge("spliter_div2").placeIn = "1";
				$ge("spliter_div2").style.display = "";
				// 删除事件对象（用于清除依赖关系）
				clearEventSimply(e);
			}
			
			this.div2.onmouseout = function(e){
				this.placeIn = "0";
				if(this.timeoutID){
					window.clearTimeout(this.timeoutID);
				}
				this.timeoutID = window.setTimeout(function(e){
					if($ge("spliter_div2").placeIn == "0"){
						$ge("spliter_div2").style.display = "none";
						$ge("spliterDivBtn").placeIn = "1";
						$ge("spliterDivBtn").style.display = "";
						blink();
						$ge("spliterFeelDiv").style.display = "";
						$ge("spliterFeelDiv").onmouseout(e);
					}
				},5000);
				// 删除事件对象（用于清除依赖关系）
			...
			this.div2.onmouseover = function(e){
				this.placeIn = "1";
				this.style.display = "";
				if(this.timeoutID){
					window.clearTimeout(this.timeoutID);
...
var blink = function(){
	...
	if($ge("spliterDivBtn").style.display == ""){
		$ge("spliterDivBtn").style.visibility = ($ge("spliterDivBtn").style.visibility == "hidden") ? "visible" : "hidden";
...
SpliterComp.prototype.fixProp = function(prop, isInverse) {
	if(!this.oneTouch && this.isRunMode){
		return;
	}
