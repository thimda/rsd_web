 ...
 */
// 表头的默认单行高度
GridComp.HEADERROW_HEIGHT = 29;
// grid数据行的默认高度
GridComp.ROW_HEIGHT = 29;
...
function GridComp(parent, name, left, top, width, height, position, editable,
		...
		oddType, isGroupWithCheckbox, isShowHeader, extendCellEditor, rowRender) {
...
GridComp.prototype.attachEvents = function() {
	...
	this.dataOuterDiv.onkeydown = function(e) {
		...
		// 右移
			...
			if (!this.isMultiSelWithBox && rowIndex != null) { // 单选表格
				var tmpCell = oThis.getCell(rowIndex, colIndex + 1);
				var tmpRowIndex = this.getCellRowIndex(tmpCell);
				if (tmpCell == null) {
...
GridComp.gridResize = function(gridId) {
	...
	try {
		...
		if(!grid.flowmode){
			if(grid.pageSize != -1)
				grid.dataOuterDiv.style.height = (grid.outerDiv.offsetHeight - GridComp.HEADERROW_HEIGHT - GridComp.PAGEBAR_HEIGHT) + "px";
			else
				grid.dataOuterDiv.style.height = (grid.outerDiv.offsetHeight - GridComp.HEADERROW_HEIGHT) + "px";
		}
...
GridComp.prototype.setCellActive = function(cell, ctrl) {
	...
	if (extendComp){
		...
		extendComp.setBounds(compOffsetLeft(cell, document.body) + GridComp.CELL_LEFT_PADDING, compOffsetTop(cell, document.body), cell.offsetWidth
...
//	} 
		...
		if (cell.editorType == null || cell.editorType == "") {
			...
			comp.setBounds(compOffsetLeft(cell, document.body) + GridComp.CELL_LEFT_PADDING,
					compOffsetTop(cell, document.body), cell.offsetWidth - GridComp.CELL_RIGHT_PADDING - 1,
					cell.offsetHeight);
			// 将真实值设置到编辑控件中
			comp.setValue(this.model.getCellValueByIndex(rowIndex, colIndex));
			comp.showV();
		...
		// 文本域类型
			...
			var comp = this.compsMap.get(cell.editorType);
			comp.setBounds(compOffsetLeft(cell, document.body) + GridComp.CELL_LEFT_PADDING,
					compOffsetTop(cell, document.body) - 1, cell.offsetWidth - GridComp.CELL_RIGHT_PADDING
							- 1, cell.offsetHeight);
			comp.setValue(this.model.getCellValueByIndex(rowIndex, colIndex));
			comp.showV();
			comp.setFocus();
...
GridComp.prototype.initPaginationBar = function() {
	...
	this.paginationInput.onenter = function(e){
		var pageIndex = parseInt(this.input.value, 10) - 1;
		GridComp.pageNavgate(e, pageIndex, oThis.id);
	}
	
//	this.firstPage = $ce("BUTTON");
////	this.firstPage.style[CSSFLOAT] = "left";
//	this.firstPage.innerHTML = "首页";
...
//	this.pageNum.style.position = "relative";
//
//	var baseWidth = 4;
//
...
GridComp.prototype.processPaginationInfo = function(pageIndex, rowCount,
	...
	if(this.paginationText3){
		this.paginationText3.innerHTML = "共" + rowCount + "条";
	}
	...
	selectedCenterDiv.innerHTML = "首页";
	...
	selectedCenterDiv.innerHTML = "末页";
				...
				if (currX > offsetLeft + 15
					...
					if (this.sortImg == null) {
						...
						this.sortImg.src = window.themePath
								+ "/ui/ctrl/grid/images/sort_up.gif";
						...
						if (header.ascending == -1) {
							this.sortImg.src = window.themePath
									+ "/ui/ctrl/grid/images/sort_down.gif";
							header.ascending = 1;
...
GridComp.prototype.adjustFixedColumDivHeight = function() {
	...
	if(!this.flowmode){
		var height = 0;
		if(this.pageSize == -1)
			height = this.outerDiv.offsetHeight - GridComp.HEADERROW_HEIGHT ;
		else
			height = this.outerDiv.offsetHeight - GridComp.HEADERROW_HEIGHT - GridComp.PAGEBAR_HEIGHT;
//			
//		if(this.isDataDivScroll())
//			height -= 10;	
		if (height > 0)
...
GridComp.addCompListener = function(grid, comp, colIndex){
	if (comp != null) {
		...
		compFocusListener.onBlur = function(keyEvent) {
			GridComp.compBlurFun(grid, keyEvent);
		};
...
GridComp.compValueChangeFun = function(oThis, valueChangeEvent) {
	...
	if (currCell) {
		...
		if (comp.oldValue != newValue || comp.componentType == "COMBOBOX") { // TODO
			...
			// cell编辑后事件
			oThis.onAfterEdit(rowIndex, colIndex, comp.oldValue,
					newValue);
...
GridComp.prototype.andLineStateAndColNum = function(rowCount, index, rowHeight,
	...
	if (this.isShowNumCol) {
		...
		$n.innerHTML = "<center>" + (index + 1) + "</ceter>";
...
//		checkDiv.rowIndex = index;
	...
	for (var i = 0; i < this.model.basicHeaders.length; i++) {
		...
		cellStyle.textAlign = "left";//header.textAlign;
		//cellStyle.color = "#0086b2";
		...
		var childNode = null;
		if (!header.isGroupBy) {
			childNode = header.renderType.render.call(this, index, i, row.getCellValue(i),
					header, cell, parentRowIndex);
		}
		// 分组显示的列
			...
			childNode = header.renderType.render.call(this, index, i, realValue, header,
					cell, parentRowIndex);
 ...
 * @private
 */
GridComp.prototype.paintZone = function(key, rowIndex, hasParent, currCell) {
	if (this.model == null)
		return;
	if (this.needPaintRows != null && this.needPaintRows == true) {
		...
		for (var i = 0; i < modelLen; i++) {
			if(hasParent){
				if(currCell.level != null){
					rows[i].level = currCell.level + 1;
...
GridComp.prototype.loadChild = function(fk, rowIndex, currCell) {
	this.paintChild(fk, rowIndex, true, currCell);
};
...
//	if (this.paintedSign[rowIndex] == 1) {
	...
	if (this.basicHeaders[colIndex].isHidden == false) {
		...
		this.basicHeaders[colIndex].renderType.render.call(this, rowIndex, colIndex,
...
GridComp.prototype.paintChild = function(key, rowIndex, hasParent, currCell) {
	this.model.initUIRows(rowIndex, null, key);
	this.paintZone(key, rowIndex, hasParent, currCell);
};
...
//	if(row != null)
//		return row;
//	for(var r in this.rowsMap){
//		var rows = this.rowsMap[r];
//		for(var i = 0; i < rows.length; i ++){
 ...
 * @private
 */
GridCompModel.prototype.initUIRows = function(startIndex, count, key) {
	// 保存UI rows的数组
	if (this.rows == null) {
		this.rows = new Array;
	}
	...
	if (RowSelectEvent.prototype.isPrototypeOf(event)) {
		...
		if (g.isMultiSelWithBox == false) {
			...
			if (event.isAdd) {
				...
				g.rowSelected(index);
//				var oldCellRowIndex = g.getCellRowIndex(g.oldCell);
				// 如果已经选中的cell和当前选中行不在同一行,则把已经选中的cell的边框去掉
//				if (g.oldCell != null && oldCellRowIndex != index) {
		...
		// 多选模式为checkbox的情况
			...
			g.rowSelected(index);
			if (g.oldCell != null){
				var rowIndex = g.getCellRowIndex(g.oldCell);
				if(rowIndex != index)
					g.oldCell.className = g.oldClassName;
			}
		}
