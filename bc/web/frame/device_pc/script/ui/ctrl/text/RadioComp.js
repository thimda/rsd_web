...
RadioComp.IMAGE_SIDE_LENGTH = 15;
 ...
 */
function RadioComp(parent, name, left, top, group, value, text, checked, position, className, attrArr) {	
	this.group = group;
	...
	this.tabIndex = -1;
	if(attrArr){
		this.radioImgSrc = getString(attrArr.imgsrc, this.radioImgSrc);
	}
	this.base = TextComp;
...
RadioComp.prototype.create = function() {
	...
	if(this.radioImgSrc){
		var radioImgDiv = $ce("DIV");
		radioImgDiv.style.marginRight = "1px";
		radioImgDiv.style.width = RadioComp.IMAGE_SIDE_LENGTH + "px";
		radioImgDiv.style.height = RadioComp.IMAGE_SIDE_LENGTH + "px";
		radioImgDiv.style[ATTRFLOAT] = "left";
		var radioImg = $ce("IMG");
		radioImg.src = window.themePath + this.radioImgSrc;
		radioImgDiv.appendChild(radioImg);
		var div = $ce("DIV");
		div.appendChild(radioImgDiv);
		this.label.Div_gen.innerHTML = div.innerHTML + this.label.Div_gen.innerHTML;	
	}
