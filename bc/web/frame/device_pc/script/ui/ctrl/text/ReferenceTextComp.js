...
//		width = width - 2;
	...
	this.refImg = $ce("IMG");
	this.refImg.src = window.themePath + '/ui/ctrl/text/images/ref_nm.png';
	...
	if(this.Div_text.children.length == 3){
		...
		if(IS_IE)
			inputWidth -= 3;
		this.input.style.width = inputWidth + "px";//2*2 input输入框距离左右间距
	}
	/********/
	// 设置图片的竖直位置居中
...
//		clearEvent(e);
	...
	this.divButton.onmouseover = function(e) {
		if (!this.disabled)	
			oThis.refImg.src = window.themePath + "/ui/ctrl/text/images/ref_on.png";
	};
	
	this.divButton.onmouseout = function(e) {
		if(!this.disabled)	
			oThis.refImg.src = window.themePath + "/ui/ctrl/text/images/ref_nm.png";
	};
...
//			oThis.hideRefDiv();
	...
	//初始控件为禁用状态
		...
		*/
		...
		this.refImg.src = window.themePath + "/ui/ctrl/text/images/ref_disable.png";
		//将与下拉按钮的各种动作事件保存
...
ReferenceTextComp.prototype.openRefDialog = function(e, isRefreshDialog) {
	if(this.nodeInfo instanceof SelfRefNodeInfo){
		...
		if (window.appType != null && window.appType == "true"){
			var appUniqueId = getAppUniqueId();
			url = window.globalPath + "/app/" + window.appId + "/" + this.widget.id + "_" + this.nodeInfo.id + "?pageId=" +  encodeURIComponent(this.nodeInfo.pageMeta) + "&widgetId=" + this.widget.id + "&otherPageId=" + getPageId() + "&otherPageUniqueId=" + getPageUniqueId() + "&nodeId=" + this.nodeInfo.id + "&owner=" + this.id + "&appUniqueId=" +appUniqueId + "&isReference=true&model=nc.uap.lfw.reference.app.AppDefaultReferencePageModel";
		}
		else{
