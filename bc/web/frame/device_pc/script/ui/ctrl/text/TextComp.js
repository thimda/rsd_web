...
TextComp.prototype.manageSelf = function() {
	...
	if (this.hasLabel)
		this.Div_text.style.width = (width - this.labelWidth - 12) < 0?0:(width - this.labelWidth - 12) + "px";
...
//	}else
	...
	if(this.Div_gen.id != 'userid' && this.Div_gen.id != 'password'){
		...
		center_div.style.width = (parseInt(this.Div_text.style.width,10) - 3*2) + "px";//3*2左右边框图片宽度
	...
	/** Ios **/
	if(IS_IOS)
		this.input.style.width = (parseInt(this.Div_text.style.width,10) - 6) + "px" ;
	else
		this.input.style.width = (parseInt(this.Div_text.style.width,10) - 10) + "px";

	/*add by chouhl 2012-1-10*/
	if(this.Div_text.children.length == 3){
		var centerWidth = parseInt(this.Div_text.style.width,10) - 3*2;//3*2左右边框图片宽度
...
TextComp.prototype.showFloatMessageDiv = function() {
	...
	else if (messageType == BaseComponent.ELEMENT_ERROR || messageType == BaseComponent.ELEMENT_WARNING) {  // 获取错误提示或警告提示
