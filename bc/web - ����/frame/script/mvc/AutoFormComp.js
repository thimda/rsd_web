...
// 额外的空白高度
AutoFormComp.SPACE_HEIGHT = 15;

AutoFormComp.ELEMENT_ERROR = "error";
AutoFormComp.ELEMENT_WARNING = "warning";
AutoFormComp.ELEMENT_NORMAL = "normal";
AutoFormComp.ELEMENT_SUCCESS = "success";
...
function AutoFormComp(parentDiv, id, renderType, renderHiddenEle, rowHeight,
	...
	// dingrf
		...
		this.pLayout = new FixedGridLayout(parentDiv, "fixedGrid_" + id, 0, 0,
				"100%", "100%", "relative", true, 120, 10, rowHeight,
				columnCount);
	}
	this.widget = null;
...
AutoFormComp.prototype.createElement = function(eleId, field, width, height,
	...
	if (type == EditorType.CHECKBOX) {
		ele = new CheckboxComp(pdiv, field, 0, 0, width, labelName, false,
				position, {
					"disabled" : disabled,
					"readOnly" : readOnly,
		...
		ele = new ReferenceTextComp(pdiv, field, 0, 0, width, position, {
			"disabled" : disabled,
			"readOnly" : readOnly,
			"tip" : tip,
			"inputAssistant" : inputAssistant
		...
		ele = new ComboComp(pdiv, field, 0, 0, width, position,
		...
		var dataDivHeight = userObject.dataDivHeight;
		ele = new ListComp(pdiv, field, 0, 0, width, dataDivHeight, false, position, className);
//		ele.setShowImgOnly(userObject.imageOnly);
		var comboData = userObject.comboData;
		if (comboData != null) {
			ele.setComboData(comboData);
		...
		ele = new RadioGroupComp(pdiv, field, 0, 0, width, position, {
			"disabled" : disabled,
			"readOnly" : readOnly,
			"inputAssistant" : inputAssistant
		});
		...
		ele = new CheckboxGroupComp(pdiv, field, 0, 0, width, position, {
			"disabled" : disabled,
			"readOnly" : readOnly,
			"inputAssistant" : inputAssistant
		});
		...
		ele = new PswTextComp(pdiv, field, 0, 0, width, position, {
			"disabled" : disabled,
			"readOnly" : readOnly,
			"inputAssistant" : inputAssistant
		}, className);
		...
		ele = new IntegerTextComp(pdiv, field, 0, 0, width, position, maxValue,
				minValue, {
					"disabled" : disabled,
					"readOnly" : readOnly,
					"tip" : tip,
		...
		ele = new FloatTextComp(pdiv, field, 0, 0, width, position,
		...
		// ImageComp(parent, name, refImg1, left, top, width, height, alt,
		// refImg2, attrObj) {
		var refImg1 = '';
		var refImg2 = '';
		...
		ele = new ImageComp(pdiv, field, refImg1, 0, 0, userObject.width,
...
//		ele = new EditorComp(pdiv, field, 0, 0, width, height, position);
	}
	// form的自定义元素
	else if (type == EditorType.SELFDEFELE) {
		ele = new SelfDefElement(pdiv, field, 0, 0, width, height, position,
				null);
		var disabled = getBoolean(disabled, true);
		ele.setActive(!disabled);
	}
	else if (type == EditorType.FILECOMP) {
		ele = new FileComp(pdiv, field, 0, 0, width, height, position, {
			"disabled" : disabled,
			"readOnly" : readOnly,
			"tip" : tip,
			"inputAssistant" : inputAssistant
		...
		ele = new StringTextComp(pdiv, field, 0, 0, width, position, {
			"disabled" : disabled,
			"readOnly" : readOnly,
			...
			if (this.dataCheckErrorRows != null
				...
				for (var i = 0; i < value[0].length; i++) {
					...
					if (this.eleArr[index].Div_text) {
						if (value[2][i] == AutoFormComp.ELEMENT_ERROR)
							this.eleArr[index].setErrorStyle();
						else if (value[2][i] == AutoFormComp.ELEMENT_WARNING)
							this.eleArr[index].setWarningStyle();
					} else if (eleHtml.className.indexOf("checkedborder") == -1)
						eleHtml.className += " checkedborder";
					if (this.eleArr[i].setErrorMessage)
						this.eleArr[i].setErrorMessage(value[1][i]);
				}
			}
		}
	}
		...
		for (var i = 0, count = colIndice.length; i < count; i++) {
			if ((eleIndex = this.indiceArr.indexOf(colIndice[i])) != -1) {
				...
				if (describes[i] != ""
					...
					// 错误类型，包括错误AutoFormComp.ELEMENT_ERROR和警告AutoFormComp.ELEMENT_WARNING
					var errorType = AutoFormComp.ELEMENT_ERROR;
					// 自定义校验结果（包含2个变量：1："type"错误类型，包括AutoFormComp.ELEMENT_ERROR和AutoFormComp.ELEMENT_WARNING；2："describe"：错误描述信息）
					...
					if (userCheckResult != null) { // 有自定义校验结果
						if (userCheckResult.type == AutoFormComp.ELEMENT_ERROR) {
							AutoFormComp.setElementErrorMessage(
									this.eleArr[eleIndex],
									AutoFormComp.ELEMENT_ERROR,
									userCheckResult.describe, this.renderType);
						} else if (userCheckResult.type == AutoFormComp.ELEMENT_WARNING) {
							AutoFormComp.setElementErrorMessage(
									this.eleArr[eleIndex],
									AutoFormComp.ELEMENT_WARNING,
									userCheckResult.describe, this.renderType);
						}
					} else { // 默认校验结果
						AutoFormComp.setElementErrorMessage(
								this.eleArr[eleIndex],
								AutoFormComp.ELEMENT_ERROR, describes[i],
								this.renderType);
					}
						...
						if (userCheckResult != null) { // 有自定义校验结果
							if (userCheckResult.type == AutoFormComp.ELEMENT_ERROR) {
								...
								if (this.eleArr[eleIndex].setCheckResult)
									this.eleArr[eleIndex]
											.setCheckResult(AutoFormComp.ELEMENT_ERROR);
							} else if (userCheckResult.type == AutoFormComp.ELEMENT_WARNING) {
								// 设置错误样式
								...
								if (this.eleArr[eleIndex].setCheckResult)
									this.eleArr[eleIndex]
											.setCheckResult(AutoFormComp.ELEMENT_WARNING);
							}
						} else {
							this.eleArr[eleIndex].setErrorStyle();
						}
					...
					if (value == null || value == "") {
						...
						AutoFormComp.setElementErrorMessage(
								this.eleArr[eleIndex],
								AutoFormComp.ELEMENT_NORMAL,
								this.eleArr[eleIndex].inputAssistant,
								this.renderType);
						// 设置校验结果类型
						if (this.eleArr[eleIndex].setCheckResult)
							this.eleArr[eleIndex]
									.setCheckResult(AutoFormComp.ELEMENT_NORMAL);
					} else {
						// 校验成功后设置成功提示
						AutoFormComp.setElementErrorMessage(
								this.eleArr[eleIndex],
								AutoFormComp.ELEMENT_SUCCESS, "",
								this.renderType);
						// 设置校验结果类型
						if (this.eleArr[eleIndex].setCheckResult)
							this.eleArr[eleIndex]
...
AutoFormComp.setElementErrorMessage = function(element, type, message,
	...
	if (renderType == 3) { // 固定提示布局类型
		...
		if (type == AutoFormComp.ELEMENT_ERROR) {
		...
		} else if (type == AutoFormComp.ELEMENT_WARNING) {
			...
			if (!IS_IE)
				...
				innerHTML = innerHTML
						...
						+ message + "</div>";
			messageDiv.innerHTML = innerHTML;
		} else if (type == AutoFormComp.ELEMENT_SUCCESS) {
			...
			var innerHTML = "<div style='float:left'><img style='height:16px;width:16px;margin-top:0px;' src='"
					...
					+ "'/></div>";
			messageDiv.innerHTML = innerHTML;
		} else if (type == AutoFormComp.ELEMENT_NORMAL) {
...
AutoFormComp.prototype.createLabel = function(comp, left, top, width) {
