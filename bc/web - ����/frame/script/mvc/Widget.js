...
//	context.javaClass = "nc.uap.lfw.core.comp.ctx.WidgetUIContext";
	...
	if (comps != null && comps.length > 0) {
		for ( var i = 0; i < comps.length; i++) {
			...
			var type = null;
			if (comp.componentType == TreeViewComp.prototype.componentType) {
				var rule = widgetRule == null ? null : widgetRule
						.getTreeRule(comp.id);
				if (rule != null)
					type = rule.type;
			}
			if (comp.componentType == AutoFormComp.prototype.componentType) {
				var rule = widgetRule == null ? null : widgetRule
						.getFormRule(comp.id);
				if (rule != null)
					type = rule.type;
			}
			if (comp.componentType == GridComp.prototype.componentType) {
				var rule = widgetRule == null ? null : widgetRule
						.getGridRule(comp.id);
				if (rule != null)
					type = rule.type;
...
Widget.prototype.replaceComboData = function(cbId, keyArr, valueArr) {
	...
	for ( var i = 0; i < comps.length; i++) {
		var comp = comps[i];
		if (comp instanceof ComboComp) {
			this.replaceCombo(comp, cbId, cb);
		} 
		else if (comp instanceof AutoFormComp) {
