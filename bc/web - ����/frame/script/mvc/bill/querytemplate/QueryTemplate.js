...
function externalInit() {
	require("label", function() {
		window.widgetId = "${widgetId}";
		window.$c_normalPanel = new QueryTemplatePanel(document
						.getElementById("$d_main_normalPanel"), "normalPanel",
				0, 0, "100%", "100%", "relative");
		...
		var toolbar_1 = new ToolBarComp(tab.rightBarSpace,
				"queryConditionToolbar", 0, 0, "90", "", "relative", null, true);
		...
		imageClearMouseListener.onclick = function(MouseEvent) {
			...
			if (pageUI.getTab('sqlTab').getSelectedIndex() == 0)
				conditionDs = pageUI.getWidget(widgetId)
						.getDataset("queryConditionDataset");
			else
				conditionDs = pageUI.getWidget(widgetId)
						.getDataset("queryConditionTreeDataset");
		...
		/* 收藏夹 */
		var toolbar_2 = new ToolBarComp(document
						.getElementById("$d_main_favoritDiv"),
				"savedConditionToolbar", 0, 0, "100%", "", "relative", null,
				false);
		...
		folderMouseListener.onclick = function(MouseEvent) {
			...
			// var dialog =
			// pageUI.getWidget(widgetId).getComponent("saveDialog");
			// dialog.show();
			// doConditionSave.flag = 'Folder';
		};
...
/**
 * 
 * function getEntityAttrs(node) { if (node.isLeaf ||
 * node.childrenTreeNodes.length > 0) return; var parentRow = node.nodeData; var
 * entityName = parentRow.getCellValue(5); var entityAttrs =
 * getService("favorite_service").getEntityAttrs(entityName, window.level++);
 * var dsResult = entityAttrs.map.dsResult; var combs = entityAttrs.map.combs;
 * var ds = pageUI.getWidget(widgetId).getDataset("queryDataset"); for ( var i =
 * 0; i < dsResult.length; i++) { var row = new DatasetRow(); for ( var j = 0; j <
 * 7; j++) { row.setCellValue(j, dsResult[i][j]); }
 *  // set parent field key row.setCellValue(7, parentRow.getCellValue(0)); //
 * set parentField row.setCellValue(9, parentRow.getCellValue(3)); // set table
 * name row.setCellValue(10, dsResult[i][7]); ds.addRow(row); }
 * 
 * for ( var i = 0; i < combs.length; i++) { var id = combs[i].id; if
 * (getComboData(id) != null) { continue; } var comb = new ComboData(); for (
 * var j = 0; j < combs[i].allCombItems.length; j++) { var item =
 * combs[i].allCombItems[j]; comb.addItem(new ComboItem(item.i18nName,
 * item.value)); } window["$cb_" + id] = comb; } }
 */
...
/**
 * 收藏夹树的叶子节点双击处理 function templateTreeSaveConditionOndbclick(node) { var
 * rightDs; if(node.isLeaf){ var row = node.nodeData; var page_fun_code =
 * parent.getParameter("pageId"); var nodeInfo = row.contentToXml() + "," +
 * page_fun_code + "," + "${templateid}";//TODO var conditionArr =
 * getService("favorite_service").loadQueryCondition(nodeInfo); var arr =
 * eval(conditionArr);
 * 
 * var array = arr[0].split(","); var tabIndex =
 * pageUI.getTab('sqlTab').getSelectedIndex();
 * 
 * //TODO //if(array[0] == '$AND_START' || array[4] == '$AND_START'){//advance
 * tab var conditionValue = arr[0].split("#")[3]; if
 * (conditionValue.toUpperCase() == "AND" || conditionValue.toUpperCase() ==
 * "OR") { //advance tab if(tabIndex == 0) pageUI.getTab('sqlTab').activeTab(1);
 * rightDs = pageUI.getWidget(widgetId).getDataset("queryConditionTreeDataset");
 * cleanDsData(false); }else{//common tab if(tabIndex == 1)
 * pageUI.getTab('sqlTab').activeTab(0); rightDs =
 * pageUI.getWidget(widgetId).getDataset("queryConditionDataset");
 * cleanDsData(true); } for(var i=0;i < arr.length;i++){ var strArr =
 * arr[i].split("#"); var addRow = new DatasetRow(); addRow.setCellValue(0,
 * strArr[0]);//id addRow.setCellValue(1, strArr[1]);//label
 * addRow.setCellValue(2, strArr[2]);//condition addRow.setCellValue(3,
 * strArr[3]);//condition_value addRow.setCellValue(4, strArr[4]);//parentId
 * addRow.setCellValue(5, strArr[5]);//editorType addRow.setCellValue(6,
 * strArr[6]);//dataType addRow.setCellValue(7, strArr[7]);//editorInfo
 * addRow.setCellValue(8, strArr[8]);//field addRow.setCellValue(9,
 * strArr[9]);//CondType addRow.setCellValue(10, strArr[10]);//editorInfo2
 * addRow.setCellValue(11, strArr[11]);//tableName addRow.setCellValue(12,
 * strArr[12]); addRow.setCellValue(13, strArr[13]); addRow.setCellValue(14,
 * strArr[14]); rightDs.addRow(addRow); rightDs.clearState(); } } }
 */

/**
 * clean ds data function cleanDsData(flag) { var rightDadaset;
 * if(flag){//common rightDadaset =
 * pageUI.getWidget(widgetId).getDataset("queryConditionDataset"); var count =
 * rightDadaset.getRowCount(); if(count != 0){ for(var k=0; k < count; k++){
 * rightDadaset.removeRow(0); } } } else{//advance rightDadaset =
 * pageUI.getWidget(widgetId).getDataset("queryConditionTreeDataset"); var count =
 * rightDadaset.getRowCount(); if(count != 0){ for(var k=0; k < count; k++){
 * //TODO ???????????????? //var tmpRow = rightDadaset.getRow(k);
 * //if(tmpRow.getCellValue(0) == '$AND_START'){ // rightDadaset.removeRow(k); //
 * break; //} rightDadaset.removeRow(0); } } } }
 */
