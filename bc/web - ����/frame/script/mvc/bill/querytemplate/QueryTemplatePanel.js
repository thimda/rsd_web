...
QueryTemplatePanel.prototype.onModelChanged = function(event) {
	...
	if (RowSelectEvent.prototype.isPrototypeOf(event)) {
		...
		this.modifyElement(event.currentRow, event.cellRowIndex,
				event.cellColIndex, event.currentValue, event.oldValue);
	}
...
QueryTemplatePanel.prototype.createElement = function(dsRow) {
	...
	if ((fieldType != 0 && fieldType != 1) && logicType != "1") {
		var img = new ImageComp(cell, "img", window.themeGlobalPath
						+ "/themes/" + window.themeId
						+ "/images/querytemplate/remove.png", 0, 0, "16", "16",
				"删除条件", null, {
					position : "relative"
				});
	...
	if (!IS_IE)
		var label = new LabelComp(cell.firstChild, "label", 0, 0, dsRow
						.getCellValue(1), "relative", null);
	else
		var label = new LabelComp(cell.firstChild, "label", 0, 6, dsRow
						.getCellValue(1), "relative", null);
	cell = row.insertCell(-1);
	cell.width = "60";
	...
	if (!editable)
		...
		condComb.onfocus = function() {
			...
			var index = ds.getCurrentRowSet().getCurrentRowData()
...
QueryTemplatePanel.prototype.modifyElement = function(row, cellRowIndex,
		cellColIndex, value, oldValue) {
	// 涓嶅垱寤烘櫘閫氳
	if (row.getCellValue(1) == "$#$")
		return;
	// 鐣ユ帀And鍜孫r琛�
	...
	if (cellColIndex != 2 && cellColIndex != 3)
		return;
	this.table.firstChild.rows[count].cells[cellColIndex].firstChild.owner
			.setValue(value);
};
 ...
 */
QueryTemplatePanel.prototype.processCondition = function(flag, row, dsRow,
		editable) {
	...
	if (flag == 2) {
		...
		valueComp = new BetweenPanel(cell, "betweenPanel", 0, 0, "100%", "20",
				"relative", type, obj);
	} else {
		if (dsRow.getCellValue(5) == EditorType.REFERENCE) {
			...
			var attr = new Object;
			valueComp = new ReferenceTextComp(cell, "valueComp", 0, 0, "100%",
					"relative", attr, nodeInfo, null, true);
			...
			valueComp = new DateTextComp(cell, "valueComp", 0, 0, "100%",
					"relative", null);
		} else if (dsRow.getCellValue(5) == EditorType.COMBOBOX) {
			var fieldId = dsRow.getCellValue(10);
			var comboData = this.widget.getComboData("comb_" + fieldId
					+ "_value");
			valueComp = new ComboComp(cell, "valueComp", 0, 0, "100%",
					"relative", true, {
						"readOnly" : false,
						"dataDivHeight" : 60
					}, null);
			...
			valueComp = new IntegerTextComp(cell, "valueComp", 0, 0, "100%",
					"relative", null, null, null, null);
		} else if (dsRow.getCellValue(5) == EditorType.DECIMALTEXT) {
			// TODO
			var precision = 2;
			valueComp = new FloatTextComp(cell, "valueComp", 0, 0, "100%",
					"relative", precision, null, null);
		} else {
			valueComp = new StringTextComp(cell, "valueComp", 0, 0, "100%",
					"relative", null, null);
		}
	}
