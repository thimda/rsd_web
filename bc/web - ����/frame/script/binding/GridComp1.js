...
GridComp.prototype.initPaginationBar = function() {
	...
	this.paginationContent.style.position = "relative";
	this.paginationContent.style[ATTRFLOAT] = "right";
	this.paginationContent.style.paddingRight = "5px";
	this.paginationContent.style.fontWeight = "bold";
...
GridComp.pageNavgate = function(e) {
	...
	if(index == grid.pageIndex)
		return;
	grid.processServerPagination(index - 1);
...
GridComp.prototype.processPaginationInfo = function(pageIndex, rowCount,
	...
	for(var i = 0; i < pageCount; i ++){
		...
		a.pageIndex = i;
		a.style.color = "#0084d0";
		a.innerHTML = (i + 1);
		a.href = "#";
		a.style.border = "1px solid #e6e7e1";
		a.gridId = this.id;
...
GridComp.prototype.andLineStateAndColNum = function(rowCount, index, rowHeight,
	...
	if (this.isShowNumCol) {
		...
		$n.innerHTML = "<center>" + rowCount + "</ceter>";
...
//		checkDiv.rowIndex = index;
	...
	for (var i = 0; i < this.model.basicHeaders.length; i++) {
		...
		if (header.dataDivWidth != null)
			cellStyle.width = (header.dataDivWidth - 4) + "px"; // 4为左右padding
		else
			cellStyle.width = (header.dataDiv.offsetWidth - 4) + "px"; // 4为左右padding
		// 如果分组显示,新分组开始的标示
 ...
 * @private
 */
GridComp.prototype.paintZone = function(key, rowIndex, hasParent) {
	if (this.model == null)
		return;
...
GridComp.prototype.loadChild = function(fk, rowIndex, currCell) {
	this.paintChild(fk, rowIndex, true);
};
...
GridComp.prototype.paintChild = function(key, rowIndex, hasParent) {
	this.model.initUIRows(rowIndex, null, key);
	this.paintZone(key, rowIndex, hasParent);
};
