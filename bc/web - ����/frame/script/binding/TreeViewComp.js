		...
		if (nodeArr[i].reload == true && !nodeArr[i].isLeaf
			...
			for ( var j = 0; j < childrenLevel.length; j++) {
				...
				var reloadNode = new TreeNode(loadNodeId, trans("ml_loading"),
						value, true, false, false, null, this.withCheckBox,this.checkBoxModel, this.canEdit);
		...
		if(currLevel.loadField != null && currLevel.loadField != ""){
			if(nodeArr[i].loadLevel == null && nodeArr[i].painted == TreeNode.NOPAINT){
				...
				if(needLoad != null && needLoad == "1"){
					...
					var reloadNode = new TreeNode(loadNodeId, trans("ml_loading"),
							value, true, false, false, null, false,this.checkBoxModel,this.canEdit);
					// 此节点绑定父级level,ds和nodeData
...
TreeViewComp.prototype.setDataset = function(dataset) {
	...
	if (this.rootNode == null) {
		var rootNode = new TreeNode("$root", this.rootCaption, "root",
				this.rootOpen, false, false, null, this.withCheckBox,this.checkBoxModel,this.canEdit);
...
TreeViewComp.prototype.createNodes = function(rows, dataset, currLevel) {
	...
	for(var i = 0; i < nodeCount; i++) {	
		...
		var node = this.nodeCreated(realNodeId, caption, value, realpNodeId, nodeData, dataset);
		if (node == null)
			continue;
...
TreeViewComp.prototype.nodeCreated = function(nodeId, caption, value, pNodeId,
	...
	if (this.isLastLevel(level))
		...
		isLeaf = false;
	var node = new TreeNode(nodeId, caption, value, false, isLeaf, "", nodeData, this.withCheckBox,this.checkBoxModel,this.canEdit);
 ...
 */
function TreeNode(id, caption, value, open, isLeaf, reload, nodeData, withCheckBox,checkBoxModel,canEdit) {
	...
	// 文件,文件夹关闭时图标
	if (this.isLeaf) {
		this.icon1 = window.themePath + "/images/treeview/leaf.gif";
		this.icon2 = this.icon1;
	} else {
		this.icon1 = window.themePath + "/images/treeview/folder_off.png";
		this.icon2 = window.themePath + "/images/treeview/folder_on.png";
	}
};
...
TreeNode.prototype.create1 = function() {
	if (this.painted == TreeNode.NOPAINT) {
		...
		divtStyle.background = "url(" + window.themePath
				+ "/images/treeview/I.gif)";
...
TreeNode.prototype.create2 = function(levelNum) {
	if (this.painted == TreeNode.PAINTHALF) {
		...
		*/
		...
		this.divRowTable.style.marginLeft = (levelNum * 19) + "px";
...
//		this.divRowTable.width = "100%";
		...
		if (this.withCheckBox) {
			...
			this.imgCheckCell.width = "19px";
			this.imgCheckCell.height = "23px";
			// checkbox
...
//					oThis.setChecked(this.checked);
		...
		this.img2Cell.width = "19px";
		this.img2Cell.height = "23px";
		// 放置文字的cell
...
//		this.textCell.appendChild(this.divText);
		...
		this.imgEdit = $ce("IMG");
		this.imgEdit.src = window.themePath + "/images/treeview/edit.gif";
		...
		this.imgDelete.src = window.themePath + "/images/treeview/delete.gif";
...
TreeNode.prototype._setCheck=function(state){
	...
	if (this.checkbox)
		this.checkbox.src = window.themePath + "/images/treeview/" + TreeViewComp.checkArray[this.checkstate];
};
...
TreeNode.prototype.create = function() {
	...
	// 创建每个节点的总div
	this.Div_gen = $ce("DIV");
	// this.Div_gen.style.border="solid black 1px"
	this.Div_gen.id = this.id;
	this.Div_gen.owner = this;
	...
	this.divRow.style[ATTRFLOAT] = "left";

	
	// guoweic: modify start 2009-11-3
	//this.divRowTable = $ce("table");
	this.divRowTable = $ce("DIV");
	this.divRow.appendChild(this.divRowTable);
	//this.divRowTable.cellSpacing = "0";
	//this.divRowTable.cellPadding = "0";
	//this.divRowTable.width = "100%";
	this.divRowTable.style.width = "100%";
	//var oTBody = $ce("tbody");
	var oTBody = $ce("DIV");
	oTBody.style.width = "100%";
	oTBody.style.height = "100%";
	this.divRowTable.appendChild(oTBody);
	//this.row = oTBody.insertRow(-1);
	this.row = $ce("DIV");
	this.row.style.width = "100%";
	this.row.style.height = "100%";
	oTBody.appendChild(this.row);
	// 加号,减号图标的cell
	//this.img1Cell = this.row.insertCell(-1);
	this.img1Cell = $ce("DIV");
	this.img1Cell.style.styleFloat = "left";
	this.row.appendChild(img1Cell);
	this.img1Cell.width = "19px";
	this.img1Cell.height = "23px";
	// 放置文件,文件夹图标的cell
	//this.img2Cell = this.row.insertCell(-1);
	this.img2Cell = $ce("DIV");
	this.img2Cell.style.styleFloat = "left";
	this.row.appendChild(img2Cell);
	this.img2Cell.width = "19px";
	this.img2Cell.height = "23px";
	// 放置文字的cell
	//this.textCell = this.row.insertCell(-1);
	this.textCell = $ce("DIV");
	this.textCell.style.styleFloat = "left";
	...
	this.divText.onmousedown = handleTreeNodeMouseDown;
	this.divText.onmousemove = handleTreeNodeMouseMove;
	this.divText.onmouseup = handleTreeNodeMouseUp;
	// 树节点的点击事件
	this.divText.onclick = handleTreeNodeClick;
	...
	this.divContent = $ce("TABLE");
	// this.divContent.style[ATTRFLOAT] = "left";//gd
	// this.divContent.style.border = "solid red 1px"
	this.Div_gen.appendChild(this.divContent);
	this.divContent.style.height = "0px";
	this.divContent.style.display = "none";

	this.divContent.cellSpacing = "0";
	this.divContent.cellPadding = "0";
	this.divContent.width = "100%";
	var cTody = $ce("tbody");
	this.divContent.appendChild(cTody);
	var row = cTody.insertRow(-1);
	this.divContent.tCell1 = row.insertCell(-1);
	this.divContent.tCell2 = row.insertCell(-1);

	this.DIVT = $ce("div");
	this.divContent.tCell1.appendChild(this.DIVT);
	var divtStyle = this.DIVT.style;
	divtStyle.display = "none";
	divtStyle.width = "19px";
	divtStyle.height = "100%";
	// divtStyle.position = "relative";
	// divtStyle[ATTRFLOAT] = "left";
	divtStyle.background = "url(" + window.themePath
			+ "/images/treeview/I.gif)";
	...
	this.divChildren = $ce("DIV");
	// this.divChildren.style.width = "100%";
	// this.divChildren.style.height = "100%";
	// this.divChildren.style.border = "1px solid blue";
	this.divContent.tCell2.appendChild(this.divChildren);

	if (!this.root) {
		this.divContent.tCell1.style.width = "19px";
		if (this.parentTreeNode.open && !this.parentTreeNode.isLeaf)
			this.parentTreeNode.divContent.style.display = 'block';
		this.parentTreeNode.divChildren.appendChild(this.Div_gen);
		if (this.refTree)
...
function handleTreeNodeClick(e) {
	var oThis = this.owner, tree = oThis.refTree;
	if (tree.isTreeActive == false)
		return;
...
//	if (e.ctrlKey) {
//		// 标示当前树处于多选模式
//		tree.multiSelectMode = true;
//		// 如果点击的节点不属于一个ds则不允许点击
//		if (tree.multiSelectNodes.length > 0) {
//			if (newNode.level.dataset.id != tree.multiSelectNodes[0].dataset.id)
//				return;
//		}
//
//		// 当前ds所有选中行索引
//		var selectedRowIndices = newNode.level.dataset.getSelectedIndices();
//		if (selectedRowIndices == null || selectedRowIndices.length == 0) {
//			tree.multiSelectNodes.push(newNode);
//			newNode.level.dataset.addRowSelected(newNode.level.dataset
//					.getRowIndex(newNode.nodeData));
//		} else {
//			// 当前点击节点index
//			var currRowIndex = newNode.level.dataset.getRowIndex(newNode.nodeData);
//			// 如果已经为选中节点则反选该节点
//			if (selectedRowIndices.indexOf(currRowIndex) != -1) {
//				tree.multiSelectNodes.removeEle(newNode);
//				newNode.level.dataset.setRowUnSelected(currRowIndex);
//			}
//			// 继续添加选中节点
//			else {
//				tree.multiSelectNodes.push(newNode);
...
//	else {
//		// 标示当前树处于单选模式
//		tree.multiSelectMode = false;
//		if (tree.multiSelectNodes.length > 0) {
//			for ( var i = 0, count = tree.multiSelectNodes.length; i < count; i++)
//				tree.multiSelectNodes[i].deSelNode();
//			tree.multiSelectNodes = [];
//		}
					...
					if (tree.levelCount > 1) 
					{
						...
						if (oldNode != null) {
							if (oldNode.root)  // 如果是根节点
								...
								oldNode.level.dataset.setRowUnSelected(oldNode.level.dataset.getRowIndex(oldNode.nodeData));
							}
						}
					} else
						index = ds.getRowIndex(oThis.nodeData);
...
TreeNode.prototype.selNode = function() {
	...
	tree.selNode = this;
//alert(this.divRow.innerHTML);
	// 改变选中节点外观
	if (this.hasChildren()) {
		if (this.divText)
			this.divText.className = "treenode_nodesel";
			//this.divRow.getChild(0).className = "treenode_left_on";
			//this.divRow.getChild(1).className = "treenode_on";
			//this.divRow.getChild(2).className = "treenode_right_on";
...
TreeNode.prototype.deSelNode = function() {
	if (this.divText){
		this.divText.className = "treenode_text";
		//this.divRow.getChild(0).className = "treenode_left_off";
		//this.divRow.getChild(1).className = "treenode_off";
		//this.divRow.getChild(2).className = "treenode_right_off";
...
TreeNode.prototype.changeChildrenIcon_backup = function() {
	...
	if (pNode.hasChildren()) {
		...
		if (pNode.painted == TreeNode.PAINTALL) {
			// 父节点是同级节点中的最后一个节点
			if (pNode.isLast()) {
				pNode.srcPlus = window.themePath + "/images/treeview/Lplus.gif";
				pNode.srcMoins = window.themePath
						+ "/images/treeview/Lminus.gif";
				if (pNode.id == this.refTree.rootNode.id)
					pNode.DIVT.style.display = "none";
				else {
					pNode.DIVT.style.background = "url(" + window.themePath
							+ "/images/treeview/I2.gif)";
					pNode.DIVT.style.display = "block";
				}
			} else {
				pNode.srcPlus = window.themePath + "/images/treeview/Tplus.gif";
				pNode.srcMoins = window.themePath
						+ "/images/treeview/Tminus.gif";
				if (pNode.id == this.refTree.rootNode.id
		...
		if (this.painted == TreeNode.PAINTALL) {
			if (this.hasChildren()) {
				...
				// 没有孩子并且没有下一个兄弟
				if (this.next() == null) {
					this.srcPlus = window.themePath + "/images/treeview/L.gif";
					this.srcMoins = window.themePath + "/images/treeview/L.gif";
				}
				// 没有孩子并且有下一个兄弟
				else {
					this.srcPlus = window.themePath + "/images/treeview/T.gif";
					this.srcMoins = window.themePath + "/images/treeview/T.gif";
				}
			}
			this.changeIcon();
		}
				...
				if(previousNode.childrenTreeNodes != null && previousNode.childrenTreeNodes.length > 0) {
					...
					if (previousNode.open == true)
						previousNode.DIVT.style.background = "url("
								+ window.themePath + "/images/treeview/I.gif)";
				} else {
					previousNode.srcPlus = window.themePath
