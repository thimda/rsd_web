...
FlowGridLayout.prototype.paint = function(isForce) {
	...
	for (var i = 0, compCount = this.comps.length; i < compCount; i++) {
		var colNum = this.comps[i].colNum; // 行号
		var rowNum = this.comps[i].rowNum; // 列号
		// 得到组件所占用的列数
		...
		if (this.comps[i].attachedLabel != null) {
			this.comps[i].attachedLabel.style.left = (usedWidth
					+ FlowGridLayout.COLSPACE - 2)
					+ "px";
			this.comps[i].attachedLabel.style.top = (this.marginTop + rowNum
