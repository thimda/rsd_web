	...
	CKEDITOR.dialog.add('templates', function(b) {
		...
		function d(k, l) {
			var m = CKEDITOR.dom.element
					.createFromHtml('<a href="javascript:void(0)" tabIndex="-1" role="option" ><div class="cke_tpl_item"></div></a>'), n = '<table style="width:350px;" class="cke_tpl_preview" role="presentation"><tr>';
			if (k.image && l)
				n += '<td class="cke_tpl_preview_img"><img src="'
						+ CKEDITOR.getUrl(l + k.image)
						+ '"'
						+ (CKEDITOR.env.ie6Compat
								? ' onload="this.width=this.width"'
								: '') + ' alt="" title=""></td>';
			n += '<td style="white-space:normal;"><span class="cke_tpl_title">'
					+ k.title + '</span><br/>';
			if (k.description)
		...
		function e(k) {
			var l = CKEDITOR.dialog.getCurrent(), m = l.getValueOf('selectTpl',
					'chkInsertOpt');
			if (m) {
			...
			contents : [{
				...
				elements : [{
					...
					children : [{
								id : 'selectTplText',
								type : 'html',
								html : '<span>' + i.selectPromptMsg + '</span>'
							}, {
								id : 'templatesList',
								type : 'html',
								focus : true,
			...
			onShow : function() {
				...
				CKEDITOR.loadTemplates(j.templates_files, function() {
							...
							if (l.length) {
								...
								g
										.setHtml('<div class="cke_tpl_empty"><span>'
												+ i.emptyListMsg
												+ '</span>'
												+ '</div>');
						});
