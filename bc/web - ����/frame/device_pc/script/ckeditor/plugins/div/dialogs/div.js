...
(function() {
	function a(d, e, f) {
		if (!e.is || !e.getCustomData('block_processed')) {
			e.is
					&& CKEDITOR.dom.element.setMarker(f, e, 'block_processed',
							true);
			d.push(e);
		}
	};
	...
	function c(d, e) {
						...
						if (/^(?!vbox|hbox)/.test(p.type)) {
							...
							if (!p.commit)
								p.commit = function(q) {
									var r = this.getValue();
									if ('dir' == p.id
											&& q.getComputedStyle('direction') == r)
										return;
									if (r)
		...
		function j(p) {
			var q = [], r = {}, s = [], t, u = p.document.getSelection(), v = u
					.getRanges(), w = u.createBookmarks(), x, y, z = p.config.enterMode == CKEDITOR.ENTER_DIV
					? 'div'
					: 'p';
			...
			for (x = 0; x < C.length; x++) {
				...
				for (A = 0; A < C[x].length; A++) {
					...
					if (!(G.getCustomData && G.getCustomData('block_processed'))) {
						G.is
								&& CKEDITOR.dom.element.setMarker(r, G,
		...
		function k(p) {
			var q = new CKEDITOR.dom.elementPath(p.getSelection()
					.getStartElement()), r = q.blockLimit, s = r
					&& r.getAscendant('div', true);
			return s;
		};
		function l(p) {
			var q = [], r = null, s, t;
		...
		function m(p) {
			var q = this.getDialog(), r = q._element && q._element.clone()
					|| new CKEDITOR.dom.element('div', d.document);
			this.commit(r, true);
			p = [].concat(p);
			var s = p.length, t;
			for (var u = 0; u < s; u++) {
		...
		var n = {}, o = [];
		return {
			title : d.lang.div.title,
			minWidth : 400,
			minHeight : 165,
			contents : [{
				id : 'info',
				label : d.lang.common.generalTab,
				title : d.lang.common.generalTab,
				elements : [{
					type : 'hbox',
					widths : ['50%', '50%'],
					children : [{
						...
						type : 'select',
						style : 'width: 100%;',
						label : d.lang.div.styleSelectLabel,
						'default' : '',
						items : [[d.lang.common.notSet, '']],
						onChange : function() {
							m.call(this, ['info:class', 'advanced:dir',
											'advanced:style']);
						},
						setup : function(p) {
							for (var q in n)
								n[q].checkElementRemovable(p, true)
										&& this.setValue(q);
						},
						commit : function(p) {
							var q;
							if (q = this.getValue()) {
								var r = n[q], s = p
										...
										|| '';
								r.applyToObject(p);
								p.setCustomData('elementStyle',
										s + r._.definition.attributes.style);
							}
						}
					}, {
						id : 'class',
				...
				title : d.lang.common.advancedTab,
				elements : [{
					type : 'vbox',
					padding : 1,
					children : [{
			...
			onLoad : function() {
				i.call(this);
				var p = this, q = this
						.getContentElement('info', 'elementStyle');
			...
			onOk : function() {
				...
				for (var q = 0; q < p; q++) {
					...
					!o[q].getAttribute('style')
							&& o[q].removeAttribute('style');
				}
				this.hide();
			},
			onHide : function() {
