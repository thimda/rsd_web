...
(function() {
	...
	var d = /^([.\d]*)+(em|ex|px|gd|rem|vw|vh|vm|ch|mm|cm|in|pt|pc|deg|rad|ms|s|hz|khz){1}?/i, e = /^(?:\b0[^\s]*\s*){1,4}$/, f = '^m{0,4}(cm|cd|d?c{0,3})(xc|xl|l?x{0,3})(ix|iv|v?i{0,3})$', g = new RegExp(f), h = new RegExp(f
			.toUpperCase()), i = {
		decimal : /\d+/,
		'lower-roman' : g,
		'upper-roman' : h,
		...
		ol : i,
		ul : j
	}, l = [[1000, 'M'], [900, 'CM'], [500, 'D'], [400, 'CD'], [100, 'C'],
			[90, 'XC'], [50, 'L'], [40, 'XL'], [10, 'X'], [9, 'IX'], [5, 'V'],
			[4, 'IV'], [1, 'I']], m = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
	function n(u) {
		u = u.toUpperCase();
		var v = l.length, w = 0;
		for (var x = 0; x < v; ++x)
		...
		utils : {
			...
			resolveList : function(u) {
				...
				if ((w = u.removeAnyChildWithName('cke:listbullet'))
					...
					u.name = 'cke:li';
					if (v.style)
						v.style = s.filters.stylesFilter([['text-indent'],
								['line-height'],
								[/^margin(:?-left)?$/, null, function(x) {
									var y = x.split(' ');
									x = CKEDITOR.tools.convertToPx(y[3] || y[1]
											|| y[0]);
									if (!p && q !== null && x > q)
										p = x - q;
									q = x;
									v['cke:indent'] = p && Math.ceil(x / p) + 1
											|| 1;
								}], [/^mso-list$/, null, function(x) {
									x = x.split(' ');
									var y = Number(x[0].match(/\d+/)), z = Number(x[1]
											.match(/\d+/));
									if (z == 1) {
										y !== r && (v['cke:reset'] = 1);
										r = y;
			...
			flattenList : function(u, v) {
				...
				for (var A = 0; A < y.length; A++) {
					...
					if (z.name in CKEDITOR.dtd.$listItem) {
						var B = z.attributes, C = z.children, D = C.length, E = C[D
								- 1];
						...
						y = y.slice(0, A).concat(z.children).concat(y.slice(A
								+ 1));
						u.children = [];
						...
						if (W)
							V.style = A([
									...
									[
											/^color$/,
											null,
											!M ? C(x.colorButton_foreStyle,
													'color') : null],
									[
											/^background-color$/,
											null,
											!M ? C(x.colorButton_backStyle,
													'color') : null]])(W, O)
									|| '';
						return null;
					},
				...
				attributes : {
					style : A(
							N ? [
									...
									[/^margin$|^margin-(?!bottom|top)/, null,
											function(O, P, Q) {
												if (P.name in {
													p : 1,
													div : 1
												}) {
													var R = x.contentsLangDirection == 'ltr'
															? 'margin-left'
															: 'margin-right';
													if (Q == 'margin')
														O = K(Q, O, [R])[R];
