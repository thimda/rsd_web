...
CKEDITOR.dialog.add('button', function(a) {
	...
	return {
		...
		onOk : function() {
			var c = this.getParentEditor(), d = this.button, e = !d, f = d
					? CKEDITOR.htmlParser.fragment.fromHtml(d.getOuterHtml()).children[0]
			...
			var h = CKEDITOR.dom.element
					.createFromHtml(g.getHtml(), c.document);
			if (e)
				c.insertElement(h);
			else {
				h.replace(d);
		...
		contents : [{
			...
			elements : [{
				...
				'default' : '',
				setup : function(c) {
					this.setValue(c.data('cke-saved-name')
							|| c.getAttribute('name') || '');
				},
				commit : b
			}, {
				...
				'default' : 'button',
				accessKey : 'T',
				items : [[a.lang.button.typeBtn, 'button'],
