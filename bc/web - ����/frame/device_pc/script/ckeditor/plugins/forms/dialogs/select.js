			...
			elements : [{
				...
				setup : function(k, l) {
					if (k == 'clear')
						...
						this.setValue(l.data('cke-saved-name')
								|| l.getAttribute('name') || '');
				},
				commit : function(k) {
					if (this.getValue())
				...
				children : [{
					...
					type : 'html',
					html : '<span>'
							+ CKEDITOR.tools.htmlEncode(a.lang.select.lines)
							+ '</span>'
				}]
			}, {
				type : 'html',
				html : '<span>'
						+ CKEDITOR.tools.htmlEncode(a.lang.select.opAvail)
						+ '</span>'
			}, {
				type : 'hbox',
				...
				children : [{
					...
					children : [{
								...
								commit : function(k) {
									var l = this.getDialog(), m = i(this), n = i(l
											.getContentElement('info',
													'cmbValue')), o = l
											.getContentElement('info',
													'txtValue').getValue();
									...
									for (var p = 0; p < m.count(); p++) {
										var q = b(k, m.getItem(p).getValue(), n
														.getItem(p).getValue(),
												l.getParentEditor().document);
								...
								setup : function(k, l) {
									...
									if (k == 'clear')
										...
										b(
												...
												m,
												m,
												n.getDialog().getParentEditor().document);
										if (l.getAttribute('selected') == 'selected')
											n.getDialog().getContentElement(
													'info', 'txtValue')
													.setValue(m);
									}
								}
					...
					children : [{
						...
						style : 'width:100%;',
						onClick : function() {
							var k = this.getDialog(), l = k.getParentEditor(), m = k
									.getContentElement('info', 'txtOptName'), n = k
									.getContentElement('info', 'txtOptValue'), o = k
									.getContentElement('info', 'cmbName'), p = k
									.getContentElement('info', 'cmbValue');
							b(o, m.getValue(), m.getValue(), k
											.getParentEditor().document);
							b(p, n.getValue(), n.getValue(), k
											.getParentEditor().document);
							m.setValue('');
							n.setValue('');
						}
						...
						style : 'width:100%;',
						onClick : function() {
							var k = this.getDialog(), l = k.getContentElement(
									'info', 'txtOptName'), m = k
									.getContentElement('info', 'txtOptValue'), n = k
									.getContentElement('info', 'cmbName'), o = k
									.getContentElement('info', 'cmbValue'), p = g(n);
							if (p >= 0) {
				...
				children : [{
					...
					onClick : function() {
						var k = this.getDialog(), l = k.getContentElement(
								'info', 'cmbValue'), m = k.getContentElement(
								'info', 'txtValue');
						m.setValue(l.getValue());
					}
				}, {
					...
					onClick : function() {
						var k = this.getDialog(), l = k.getContentElement(
								'info', 'cmbName'), m = k.getContentElement(
								'info', 'cmbValue'), n = k.getContentElement(
								'info', 'txtOptName'), o = k.getContentElement(
								'info', 'txtOptValue');
						c(l);
						c(m);
						n.setValue('');
