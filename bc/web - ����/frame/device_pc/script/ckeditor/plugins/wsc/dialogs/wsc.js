...
CKEDITOR.dialog.add('checkspell', function(a) {
	var b = CKEDITOR.tools.getNextNumber(), c = 'cke_frame_' + b, d = 'cke_data_'
			+ b, e = 'cke_error_' + b, f, g = document.location.protocol
			|| 'http:', h = a.lang.spellCheck.notAvailable, i = '<textarea style="display: none" id="'
			+ d
			+ '"'
			+ ' rows="10"'
			+ ' cols="40">'
			...
			+ c
			+ '"'
			+ ' allowtransparency="1">' + '</iframe>', j = a.config.wsc_customLoaderScript
			|| g
	...
	if (a.config.wsc_customLoaderScript)
		h += '<p style="color:#000;font-size:11px;font-weight: normal;text-align:center;padding-top:10px">'
				+ a.lang.spellCheck.errorLoading.replace(/%s/g,
	...
	function l(m) {
		var n = new window._SP_FCK_LangCompare(), o = CKEDITOR
				.getUrl(a.plugins.wsc.path + 'dialogs/'), p = o
				+ 'tmpFrameset.html';
	...
	return {
		title : a.config.wsc_dialogTitle || a.lang.spellCheck.title,
		minWidth : 485,
		minHeight : 380,
		...
		onShow : function() {
			...
			if (typeof window.doSpell != 'function')
				CKEDITOR.document.getHead().append(CKEDITOR.document
		...
		contents : [{
					id : 'general',
					label : a.config.wsc_dialogTitle || a.lang.spellCheck.title,
					padding : 0,
					elements : [{
								type : 'html',
								id : 'content',
...
CKEDITOR.dialog.on('resize', function(a) {
			...
			if (c._.name == 'checkspell') {
				var d = c.getContentElement('general', 'content').getElement(), e = d
						&& d.getChild(2);
