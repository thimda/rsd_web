...
CKEDITOR.dialog.add('smiley', function(a) {
	var b = a.config, c = a.lang.smiley, d = b.smiley_images, e = b.smiley_columns
			|| 8, f, g, h = function(o) {
		...
		if (q == 'a')
			...
			return;
		var r = p.getAttribute('cke_src'), s = p.getAttribute('title'), t = a.document
				.createElement('img', {
							attributes : {
								src : r,
		...
		o.data.preventDefault();
			...
			}), j = CKEDITOR.tools.getNextId() + '_smiley_emtions_label', k = [
			'<div><span id="' + j + '" class="cke_voice_label">' + c.options
					+ '</span>',
		...
		k
				.push(
						...
						CKEDITOR.env.ie
								...
								: '',
						'><span id="' + m + '" class="cke_voice_label">'
								+ b.smiley_descriptions[f] + '</span>' + '</a>',
						'</td>');
		if (f % e == e - 1)
			k.push('</tr>');
	}
