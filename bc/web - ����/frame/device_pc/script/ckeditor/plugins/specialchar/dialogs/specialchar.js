...
CKEDITOR.dialog.add('specialchar', function(a) {
	var b, c = a.lang.specialChar, d = function(j) {
		...
		if (k.getName() == 'span')
			k = k.getParent();
		if (k.getName() == 'a') {
			b.getContentElement('info', 'charPreview').getElement()
					.setHtml('&nbsp;');
			b.getContentElement('info', 'htmlPreview').getElement()
					.setHtml('&nbsp;');
	...
	return {
		...
		charColumns : 17,
		onLoad : function() {
			var j = this.definition.charColumns, k = a.config.extraSpecialChars, l = a.config.specialChars, m = CKEDITOR.tools
					.getNextId()
					+ '_specialchar_table_label', n = ['<table role="listbox" aria-labelledby="'
					+ m
					+ '"'
				...
				for (var s = 0; s < j; s++, o++) {
					if (q = l[o]) {
						...
						if (q instanceof Array) {
							...
							var t = q.replace('&', '').replace(';', '')
									.replace('#', '');
							r = c[t] || q;
						}
						var u = 'cke_specialchar_label_' + o + '_'
			...
			n.push('</tbody></table>', '<span id="' + m
							+ '" class="cke_voice_label">' + c.options
							+ '</span>');
			this.getContentElement('info', 'charContainer').getElement()
					.setHtml(n.join(''));
		},
		contents : [{
			id : 'info',
			...
			elements : [{
				...
				children : [{
					...
					onMouseout : h,
					focus : function() {
						var j = this.getElement().getElementsByTag('a')
								.getItem(0);
						setTimeout(function() {
									j.focus();
									g(null, j);
