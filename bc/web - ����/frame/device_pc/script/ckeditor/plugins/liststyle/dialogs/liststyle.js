...
(function() {
	...
	function d(e, f) {
		...
		if (f == 'bulletedListStyle')
			return {
				...
				contents : [{
					...
					elements : [{
						...
						align : 'center',
						style : 'width:150px',
						items : [[g.notset, ''], [g.circle, 'circle'],
								[g.disc, 'disc'], [g.square, 'square']],
						setup : function(i) {
							var j = i.getStyle('list-style-type')
			...
			var h = [[g.notset, ''], [g.lowerRoman, 'lower-roman'],
					[g.upperRoman, 'upper-roman'],
					[g.lowerAlpha, 'lower-alpha'],
					[g.upperAlpha, 'upper-alpha'], [g.decimal, 'decimal']];
			if (!CKEDITOR.env.ie || CKEDITOR.env.version > 7)
				h
						.concat([[g.armenian, 'armenian'],
					...
					elements : [{
						...
						children : [{
							...
							validate : CKEDITOR.dialog.validate
									.integer(g.validateStartNumber),
							setup : function(i) {
								var j = i.getFirst(b).getAttribute('value')
										|| i.getAttribute('start') || 1;
								j && this.setValue(j);
							},
							commit : function(i) {
								var j = i.getFirst(b), k = j
										.getAttribute('value')
										|| i.getAttribute('start') || 1;
