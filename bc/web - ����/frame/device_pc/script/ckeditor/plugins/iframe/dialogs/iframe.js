...
(function() {
	...
	function c(d) {
		var h = this;
		var e = h.getValue() === '', f = h instanceof CKEDITOR.ui.dialog.checkbox, g = h
				.getValue();
		if (e)
	...
	CKEDITOR.dialog.add('iframe', function(d) {
				...
				return {
					...
					onShow : function() {
						...
						if (h && h.data('cke-real-element-type')
								&& h.data('cke-real-element-type') == 'iframe') {
							j.fakeImage = h;
					...
					onOk : function() {
						...
						var k = d.createFakeElement(h, 'cke_iframe', 'iframe',
								true);
						k.setAttributes(j);
						k.setStyles(i);
						if (l.fakeImage) {
					...
					contents : [{
						...
						elements : [{
							...
							children : [{
								...
								label : f.width,
								validate : CKEDITOR.dialog.validate
										.htmlLength(f.invalidHtmlLength
												.replace('%1', f.width)),
								setup : b,
								commit : c
							}, {
								...
								type : 'select',
								'default' : '',
								items : [[f.notSet, ''], [f.alignLeft, 'left'],
										[f.alignRight, 'right'],
										[f.alignTop, 'top'],
