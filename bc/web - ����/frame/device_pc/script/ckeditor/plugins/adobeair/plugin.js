				...
				for (var i = 0; i < a.length; i++)
					(function(j) {
						...
						if (h.hasAttribute('on' + j)) {
							...
							h.on(j, function(l) {
								var m = /(return\s*)?CKEDITOR\.tools\.callFunction\(([^)]+)\)/
										.exec(k), n = m && m[1], o = m
										&& m[2].split(','), p = /return false;/
										.test(k);
								if (o) {
									var q = o.length, r;
									for (var s = 0; s < q; s++) {
									...
									var u = CKEDITOR.tools.callFunction.apply(
											window, o);
									if (n && u === false)
										p = 1;
								}
...
CKEDITOR.dom.document.prototype.write = CKEDITOR.tools.override(
		CKEDITOR.dom.document.prototype.write, function(a) {
			function b(c, d, e, f) {
				var g = c.append(d), h = CKEDITOR.htmlParser.fragment
						.fromHtml(e).children[0].attributes;
				h && g.setAttributes(h);
			...
			return function(c, d) {
				if (this.getBody()) {
					...
					c.replace(/(<body[^>]*>)([\s\S]*)(?=$|<\/body>)/i,
							function(g, h, i) {
								e.getBody().setHtml(i);
								var j = CKEDITOR.htmlParser.fragment
										.fromHtml(h).children[0].attributes;
