...
(function() {
	...
	function d(e, f) {
		...
		return {
			...
			onLoad : function() {
				...
				if (j)
					j.on('change', function(k) {
								var l = this.getStyle('width', ''), m = i
										.getContentElement('info', 'txtWidth');
								m && m.setValue(l, true);
								var n = this.getStyle('height', ''), o = i
			...
			onShow : function() {
				var q = this;
				var i = e.getSelection(), j = i.getRanges(), k = null, l = q
						.getContentElement('info', 'txtRows'), m = q
						.getContentElement('info', 'txtCols'), n = q
						.getContentElement('info', 'txtWidth'), o = q
						.getContentElement('info', 'txtHeight');
			...
			onOk : function() {
				var i = e.getSelection(), j = this._.selectedElement
						&& i.createBookmarks(), k = this._.selectedElement
						|| g('table'), l = this, m = {};
				this.commitContent(m, k);
				if (m.info) {
					var n = m.info;
					if (!this._.selectedElement) {
						var o = k.append(g('tbody')), p = parseInt(n.txtRows,
								10)
								|| 0, q = parseInt(n.txtCols, 10) || 0;
						for (var r = 0; r < p; r++) {
							var s = o.append(g('tr'));
							for (var t = 0; t < q; t++) {
								var u = s.append(g('td'));
					...
					if (!k.$.tHead && (v == 'row' || v == 'both')) {
						...
						for (r = 0; r < x.getChildCount(); r++) {
							var y = x.getChild(r);
							if (y.type == CKEDITOR.NODE_ELEMENT
									&& !y.data('cke-bookmark')) {
								y.renameNode('th');
					...
					n.txtHeight ? k.setStyle('height', n.txtHeight) : k
							.removeStyle('height');
					n.txtWidth ? k.setStyle('width', n.txtWidth) : k
							.removeStyle('width');
				...
				elements : [{
					...
					children : [{
						...
						children : [{
							...
							validate : function() {
								...
								i = i && CKEDITOR.dialog.validate.integer()(j)
										&& j > 0;
								if (!i) {
							...
							items : [[e.lang.table.headersNone, ''],
							...
							setup : function(i) {
								...
								if (i.$.tHead !== null)
									this.setValue(j.hasColumnHeaders
											? 'both'
											: 'row');
								else
									this.setValue(j.hasColumnHeaders
							...
							validate : CKEDITOR.dialog.validate
									.number(e.lang.table.invalidBorder),
							setup : function(i) {
						...
						children : [{
							...
							children : [{
								...
								title : e.lang.common.cssLengthTooltip,
								'default' : 500,
								getValue : a,
								validate : CKEDITOR.dialog.validate
										.cssLength(e.lang.common.invalidCssLength
												.replace('%1',
														e.lang.common.width)),
								onChange : function() {
									var i = this.getDialog().getContentElement(
											'advanced', 'advStyles');
									i
											&& i.updateStyle('width', this
															.getValue());
								},
								setup : function(i) {
							...
							'default' : 1,
							validate : CKEDITOR.dialog.validate
									.number(e.lang.table.invalidCellSpacing),
							setup : function(i) {
							...
							commit : function(i, j) {
								if (this.getValue())
									j.setAttribute('cellSpacing', this
													.getValue());
								else
							...
							'default' : 1,
							validate : CKEDITOR.dialog.validate
									.number(e.lang.table.invalidCellPadding),
							setup : function(i) {
							...
							commit : function(i, j) {
								if (this.getValue())
									j.setAttribute('cellPadding', this
													.getValue());
								else
					...
					children : [{
						...
						setup : function(i) {
							...
							if (j.count() > 0) {
								var k = j.getItem(0), l = k
										.getFirst(CKEDITOR.dom.walker
						...
						commit : function(i, j) {
							...
							if (k) {
								if (l.count() > 0) {
									...
									l = new CKEDITOR.dom.element('caption',
											e.document);
									if (j.getChildCount())
