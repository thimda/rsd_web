	...
	var a = function(b, c) {
		...
		var p = function(C, D) {
			...
			if (D == 'check') {
				if (!C.userlockRatio && E.getCustomData('isReady') == 'true') {
					var F = C.getValueOf('info', 'txtWidth'), G = C.getValueOf(
							'info', 'txtHeight'), H = E.$.width * 1000
							/ E.$.height, I = F * 1000 / G;
					C.lockRatio = false;
					if (!F && !G)
						C.lockRatio = true;
			...
			if (D.getCustomData('isReady') == 'true') {
				var E = C.getContentElement('info', 'txtWidth'), F = C
			...
			var F = this.getDialog(), G = '', H = this.id == 'txtWidth'
					? 'width'
		...
		return {
			...
			onShow : function() {
				...
				if (E && E.getName() == 'img' && !E.data('cke-realelement')
						|| E && E.getName() == 'input'
			...
			contents : [{
				...
				elements : [{
					...
					children : [{
						...
						children : [{
							...
							setup : function(C, D) {
								if (C == d) {
									var E = D.data('cke-saved-src')
											|| D.getAttribute('src'), F = this;
						...
						children : [{
							...
							children : [{
								...
								children : [{
									...
									validate : function() {
										var C = this.getValue().match(i), D = !!(C && parseInt(
												C[1], 10) !== 0);
										if (!D)
								...
								onLoad : function() {
									var C = CKEDITOR.document.getById(x), D = CKEDITOR.document
											.getById(w);
									if (C) {
				...
				elements : [{
					...
					setup : function(C, D) {
						if (C == d) {
							...
							if (!E && D.$.style.cssText)
								E = D.$.style.cssText;
							this.setValue(E);
							var F = D.$.style.height, G = D.$.style.width, H = (F
									? F
									: '').match(h), I = (G ? G : '').match(h);
					...
					onChange : function() {
						o.call(this, ['info:cmbFloat', 'info:cmbAlign',
										'info:txtVSpace', 'info:txtHSpace',
										'info:txtBorder', 'info:txtWidth',
										'info:txtHeight']);
						l(this);
					},
					commit : function(C, D) {
