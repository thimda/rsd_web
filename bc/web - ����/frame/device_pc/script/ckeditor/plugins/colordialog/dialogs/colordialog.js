...
CKEDITOR.dialog.add('colordialog', function(a) {
	...
	var l = d.addFunction(k), m = i, n = CKEDITOR.tools.addFunction(m), o = j, p = k, q = CKEDITOR.tools
	...
	function r() {
		...
		function B(F, G) {
			...
			H
					.append(CKEDITOR.dom.element
							.createFromHtml(
									'<a href="javascript: void(0);" role="option" aria-posinset="'
											+ I
											+ '"'
											+ ' aria-setsize="'
											+ 234
											+ '"'
											+ ' style="cursor: pointer;display:block;width:100%;height:100% " title="'
											+ CKEDITOR.tools.htmlEncode(G)
											+ '"'
											...
											+ ' tabindex="-1"><span class="cke_voice_label">'
											+ G + '</span>&nbsp;</a>',
									CKEDITOR.document));
		};
		A(0, 0);
	...
	return {
		...
		contents : [{
			...
			elements : [{
				...
				children : [{
					type : 'html',
					html : '<table role="listbox" aria-labelledby="' + y
							+ '" onmouseout="CKEDITOR.tools.callFunction( ' + l
							+ ' );">' + (!CKEDITOR.env.webkit ? t : '')
							+ '</table><span id="' + y
							+ '" class="cke_voice_label">' + e.options
							+ '</span>',
					onLoad : function() {
					...
					focus : function() {
						var z = this.getElement().getElementsByTag('a')
								.getItem(0);
						z.focus();
					}
				}, g, {
					...
					children : [{
						...
						onChange : function() {
							try {
								c.getById(x).setStyle('background-color',
										this.getValue());
							} catch (z) {
								h();
							}
