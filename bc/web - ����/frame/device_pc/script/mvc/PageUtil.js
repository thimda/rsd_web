...
function getMasker(type) {
	if(type == IntegerTextComp.prototype.componentType){
		return new NumberMasker(window.$maskerMeta.NumberFormatMeta);
	}
	if(type == DateTextComp.prototype.componentType){
...
function showErrorDialog(msg) {
	return ErrorDialogComp.showDialog(msg);
};
...
function showMessageDialog(msg) {
	return MessageDialogComp.showDialog(msg);
};
...
//		parent.showDialog.trueParent = window;
		parent.showDialog.dialogsTrueParent[parent.showDialog.dialogCount] = window; 
		parent.showDialog(pageUrl, title, width, height, id, refresh,
				clearState);
		return;
	}
...
function handleExceptionByDoc(doc, exception, ajaxArgs, ajaxObj, xmlHttpReq) {
	if (exception) {
		if (xmlHttpReq != null && xmlHttpReq.status == 306) {  // 如果是会话失效，则跳转到登陆页
			...
			showExceptionDialog(exception, exception, exception);
		}
		return false;
	}
	...
	if (success == "false") {
		...
		showExceptionDialog(showMessage, expText, expStack);
		return false;
	} 
...
//		return window.isContinuePost;
		...
		if(interationInfo.type == "OKCANCEL_DIALOG") {
			...
			showConfirmDialog(msg, rePostOk, rePostCancel);
		}
		else if(interationInfo.type == "THREE_BUTTONS_DIALOG") {
			var msg = interationInfo.msg;
			rePostReq.ajaxObj = ajaxObj;
			ThreeButtonsDialog.showDialog(msg, rePostOk, rePostCancel, rePostMiddle, interationInfo.btnTexts);
		}
 ...
 * @return
 */
function showMessage(title,width){
	MessageComp.showMessage("top-center",title,width,"0","30","")
};
 ...
 * 显示确认对话框
 */
function showConfirmDialog(msg, okFunc, cancelFunc, obj1, obj2, zIndex) {
	ConfirmDialogComp.showDialog(msg, okFunc, cancelFunc);
};
