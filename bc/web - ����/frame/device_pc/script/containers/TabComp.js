...
TabItem.prototype.create = function() {
	...
	// 得到文字的宽度
	var textWidth = getTextWidth(this.title, this.parentTab.className
			+ "_text_width", true)
			+ 2;
	this.tabItemDiv.className = this.tabType == TabComp.TYPE_TOP ? "tab_item_div" : "tab_item_div_bottom";

	// 创建每个tab的左边界显示效果
	this.borderLeft = $ce("DIV");
	...
	if (this.showCloseIcon && this.closeImgDiv == null) {
		...
		if (this.isActive)
			this.closeImgDiv.className = "tab_item_closeicon_on";
		else
			this.closeImgDiv.className = "tab_item_closeicon";
...
//			oThis.parentTab.removeItemTab(oThis.parentTab.getItemIndex(oThis));
		...
		this.closeImgDiv.onmouseover = function(e) {
			this.className = "tab_item_closeicon_over";
		};
		this.closeImgDiv.onmouseout = function(e) {
			if (oThis.isActive)
				this.className = "tab_item_closeicon_on";
			else
				this.className = "tab_item_closeicon";
		};
	}
