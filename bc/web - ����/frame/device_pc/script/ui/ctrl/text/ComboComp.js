...
//按钮图片宽高
ComboComp.prototype.imageWidth = 15;
ComboComp.prototype.imageHeight = 16;
...
ComboComp.prototype.manageSelf = function() {
	...
	if (width == 0 && this.Div_text.style.width != null){
		if (this.Div_text.style.width.indexOf("px") > -1)
			width = this.Div_text.style.width.replace("px", "");
	}
...
//			width = width - 5;
	...
	//TODO guoweic: this.width若为100%时有问题
	...
	this.pushImg = $ce("IMG");
	this.pushImg.src = window.themePath + "/ui/ctrl/text/images/combo_nm.gif";
	...
	if (this.readOnly != true) {
		...
		this.divButton.onmouseover = function(e) {
			e = EventUtil.getEvent();
			oThis.pushImg.src = window.themePath
					+ "/ui/ctrl/text/images//combo_on.gif";
			// 删除事件对象（用于清除依赖关系）
			clearEventSimply(e);
		...
		this.divButton.onmouseout = function(e) {
			e = EventUtil.getEvent();
			oThis.pushImg.src = window.themePath
					+ "/ui/ctrl/text/images/combo_nm.gif";
			// 删除事件对象（用于清除依赖关系）
			clearEventSimply(e);
		};
	} else {
...
//	this.dataDiv.style.zIndex = BaseComponent.STANDARD_ZINDEX + 1000000000;
	...
	if (this.disabled == true) {
		...
		this.pushImg.src = window.themePath
				+ "/ui/ctrl/text/images/combo_disable.gif";
	}
...
ComboComp.prototype.setBounds = function(left, top, width, height) {
	...
	var pWidth = this.Div_text.offsetWidth;
...
//			pWidth = pWidth - 5;
	...
	this.input.style.width = pWidth - this.imageWidth + "px";
	if (IS_IE7)
		//this.divButton.style.left = pWidth - 18 + "px";
	/*add by chouhl 2012-1-10*/
	if(this.Div_text.children.length == 3){
		var centerWidth = pWidth - 3*2;//3*2左右边框图片宽度
		this.Div_text.children[1].style.width = centerWidth + "px";
		var imgWidth = (this.Div_text.children[1].children.length - 1) * 18;//与input输入框同一个DIV中图片的总宽度
		this.input.style.width = (centerWidth - 2*2 - imgWidth) + "px";//2*2 input输入框距离左右间距
	}
	/********/
};
...
ComboComp.prototype.setComboData = function(comboData) {
	...
	if (nameArr != null) {
		for ( var i = 0; i < nameArr.length; i++) {
			var selected = false;
			this.createOption(nameArr[i], valueArr[i], null, selected, -1,
					this.showImgOnly);
		}
	}
