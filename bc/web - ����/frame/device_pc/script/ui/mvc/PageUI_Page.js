...
function PageUI(title) {
	...
	// 界面上包括的所有菜单组
	this.menubarGroupMap = new HashMap;
	// 界面上包括的所有未放入菜单组的菜单
	this.menubarMap = new HashMap();
	this.pageState = null;
 ...
 * @private
 */
PageUI.prototype.addMenubarGroup = function(menubarGroup) {
	if (menubarGroup != null)
		this.menubarGroupMap.put(menubarGroup.id, menubarGroup);
};

/**
 * 根据ID获取Menubar
 */
PageUI.prototype.getMenubar = function(id) {
	var menubar = this.menubarMap.get(id);
	if (menubar == null && this.menubarGroupMap != null) {
		var menubarGroups = this.menubarGroupMap.values();
		for (var i = 0; i < menubarGroups.length; i++) {
			var menubars = menubarGroups[i].menubars.values();
			for (var j = 0; j < menubars.length; j++) {
				if (menubars[j].id == id) {
					return menubars[j];
				}
			}
 ...
 */
PageUI.prototype.getActivedMenubars = function() {
	var menubars = [];
	// 各个menubargroup中当前显示的字段
	if (this.menubarGroupMap != null) {
		for ( var i = 0, count = this.menubarGroupMap.values().length; i < count; i++) {
			var group = this.menubarGroupMap.values()[i];
			if (group.nowMenubar != null)
				menubars.push(group.nowMenubar);
		}
	}
	// 当前可见的menubar
	for ( var i = 0, count = this.menubarMap.values().length; i < count; i++) {
		//TODO:判断当前menubar是否可见
		menubars.push(this.menubarMap.values()[i]);
	}
	return menubars;
};
 ...
 * @private
 */
PageUI.prototype.addMenubar = function(menubar) {
	if (menubar != null)
		this.menubarMap.put(menubar.id, menubar);
};
...
PageUI.prototype.getContext = function(submitRule) {
	...
	var menubars = this.getActivedMenubars();
	if (menubars != null) {
		for ( var i = 0; i < menubars.length; i++) {
			var menubar = menubars[i];
			contextXmlArray.push("<menubar id=\"" + menubar.id + "\">");
			contextXmlArray.push("<" + EventContextConstant.context + "><![CDATA[");
			contextXmlArray.push(toJSON(menubar.getContext()));
			contextXmlArray.push("]]></" + EventContextConstant.context + ">");
...
PageUI.prototype.setPageState = function(state) {
	...
	var size = this.menubarGroupMap.size();
	if (size > 0) {
		var values = this.menubarGroupMap.values();
		for ( var i = 0; i < size; i++)
			values[i].setState(state);
	}
	this.updateButtons();
};
		...
		for ( var n = 0, count = activedMenubars.length; n < count; n++) {
			...
			if (menubarItems != null) {
				...
				for ( var i = 0, count1 = menuItems.length; i < count1; i++) {
					if (menuItems[i].childMenu != null) {
						var cItems = menuItems[i].childMenu.childItems;
...
PageUI.prototype.handleHotKey = function(key) {
	...
	if (menubars != null) {
		for ( var i = 0; i < menubars.length; i++) {
			var menubar = menubars[i];
			var obj = menubar.handleHotKey(key);
			if (obj != null)
				return obj;
		}
	}
...
// * 设置主widget
// */
// PageUI.prototype.setMasterWidget = function(masterWidgetId) {
// this.masterWidgetId = masterWidgetId;
// };

// /**
// * 获取主widget
// */
// PageUI.prototype.getMasterWidget = function() {
// return this.masterWidgetId;
// };
...
PageUI.prototype.showCloseConfirm = function() {
	if (this.hasChanged == true) {
		window.hasCloseConfirmed = true;
		var result = confirm('页面发生改变，是否强行关闭？');
		if (result == false) {
			return false;
		} else {
			return true;
...
ServerProxy.prototype.execute = function() {
	...
	// 拼接Context内容
	var contextXmlArray = new Array();
	contextXmlArray.push("<root><" + EventContextConstant.eventcontext + ">");
...
//					//小数精度变大时，要在更新dataset前先更新一下cmop的precision,否则会截短数据小数值
