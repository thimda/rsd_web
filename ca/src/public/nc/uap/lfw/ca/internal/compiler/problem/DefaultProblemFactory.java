...
import nc.uap.lfw.ca.jdt.internal.compiler.util.HashtableOfInt;
...
public static HashtableOfInt loadMessageTemplates(Locale loc) {
	...
	String bundleName = ConstantUtils.BundleName; //$NON-NLS-1$
	try {
		bundle = ResourceBundle.getBundle(bundleName, loc); 
	} catch(MissingResourceException e) {
		LfwLogger.error("Missing resource : " + bundleName.replace('.', '/') + ".properties for locale " + loc); //$NON-NLS-1$//$NON-NLS-2$
		throw e;
	}
