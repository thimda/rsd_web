...
public void addInnerEmulationDependent(BlockScope dependentScope, boolean wasEnclosingInstanceSupplied) {
	...
	//  LfwLogger.error("Adding dependency: "+ new String(scope.enclosingType().readableName()) + " --> " + new String(this.readableName()));
}
...
public void updateInnerEmulationDependents() {
	if (this.dependents != null) {
		for (int i = 0; i < this.dependents.length; i++) {
			...
			// LfwLogger.error("Updating " + new String(this.readableName()) + " --> " + new String(dependency.scope.enclosingType().readableName()));
