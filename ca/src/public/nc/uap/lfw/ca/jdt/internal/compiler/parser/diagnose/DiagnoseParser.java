 ...
 *******************************************************************************/
package nc.uap.lfw.ca.jdt.internal.compiler.parser.diagnose;
...
import nc.uap.lfw.ca.jdt.internal.compiler.parser.LexStream;
import nc.uap.lfw.ca.jdt.internal.compiler.parser.Parser;
import nc.uap.lfw.ca.jdt.internal.compiler.parser.ParserBasicInformation;
				...
				while(act > ERROR_ACTION || act < ACCEPT_ACTION) { // SHIFT-REDUCE action or SHIFT action ?
					...
					while(act <= NUM_RULES) {
						...
						do {
							...
							if(DEBUG) {
								LfwLogger.debug(Parser.name[Parser.non_terminal_index[lhs_symbol]]);
							}
							tempStackTop -= (Parser.rhs[act]-1);
...
//
	private int parseCheck(int stck[], int stack_top, int first_token, int buffer_position) {
		...
		if (first_token > NT_OFFSET) {
			...
			if(DEBUG_PARSECHECK) {
				LfwLogger.debug(String.valueOf(tempStackTop));
			}
			max_pos = stack_top;
			...
			if (act <= NUM_RULES) {
				...
				do {
					...
					if(DEBUG_PARSECHECK) {
						...
						LfwLogger.debug(String.valueOf(-(Parser.rhs[act]-1)));
						LfwLogger.debug(") [max:"); //$NON-NLS-1$
						LfwLogger.debug(String.valueOf(max_pos));
						LfwLogger.debug("]\tprocess_non_terminal\t"); //$NON-NLS-1$
						LfwLogger.debug(String.valueOf(act));
						LfwLogger.debug("\t"); //$NON-NLS-1$
						LfwLogger.debug(Parser.name[Parser.non_terminal_index[Parser.lhs[act]]]);
						LfwLogger.debug("");
					}
		...
		process_terminal: for (;;) {
			if(DEBUG_PARSECHECK) {
				LfwLogger.debug(String.valueOf(tempStackTop + 1));
				LfwLogger.debug(" (+1) [max:"); //$NON-NLS-1$
				LfwLogger.debug(String.valueOf(max_pos));
				LfwLogger.debug("]\tprocess_terminal    \t"); //$NON-NLS-1$
				LfwLogger.debug(String.valueOf(ct));
				LfwLogger.debug("\t"); //$NON-NLS-1$
				LfwLogger.debug(Parser.name[Parser.terminal_index[ct]]);
				LfwLogger.debug("");
			}
			...
			if (act <= NUM_RULES) {               // reduce action
				...
				if (act > ERROR_ACTION) {
					...
					if(DEBUG_PARSECHECK) {
						LfwLogger.debug(String.valueOf(tempStackTop));
						LfwLogger.debug("\tshift reduce"); //$NON-NLS-1$
						LfwLogger.debug("");
					}
				} else {
					if(DEBUG_PARSECHECK) {
						LfwLogger.debug("\tshift"); //$NON-NLS-1$
					}
