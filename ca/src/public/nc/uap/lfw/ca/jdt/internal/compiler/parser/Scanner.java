 ...
 * eclipse.jdt.core.compiler public package where it is API. 
...
public final void pushLineSeparator() {
	...
	if (this.currentCharacter == '\r') {
		...
		try {
			if (this.source[this.currentPosition] == '\n') {
				//LfwLogger.debug("look-ahead LF-" + this.currentPosition);			
		...
		if (this.currentCharacter == '\n') { //must merge eventual cr followed by lf
			if (this.wasAcr && (this.lineEnds[this.linePtr] == (this.currentPosition - 2))) {
				//LfwLogger.debug("merge LF-" + (this.currentPosition - 1));							
