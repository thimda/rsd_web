...
function externalInit() {
	...
	var toolbar_1 = new ToolBarComp(tab.rightBarSpace, "queryConditionToolbar", 0, 0, "90", "", "relative", null, true);
	toolbar_1.addButton("imageSave", "", "${ml:transp('lfw','query_save')}", window.themeGlobalPath + "/themes/" + window.themeId + "/images/querytemplate/save.png", "right", true);
	toolbar_1.addButton("imageClear", "", "${ml:transp('lfw','query_clean')}", window.themeGlobalPath + "/themes/" + window.themeId + "/images/querytemplate/clear.png", "right", true);
	toolbar_1.addButton("imageReset", "", "${ml:transp('lfw','query_reset')}", window.themeGlobalPath + "/themes/" + window.themeId + "/images/querytemplate/reset.png", "right", true);
	// 保存
	...
	imageSaveMouseListener.onclick = function(MouseEvent) {
		...
		if (!window.$c_conditionSavedialog) {
			window.$c_conditionSavedialog = new InputDialogComp("saveText", "输入对话框", 0, 0, null, null, 100, doConditionSave);
	...
	imageClearMouseListener.onclick = function(MouseEvent) {
		...
		if(pageUI.getTab('sqlTab').getSelectedIndex() == 0)
			conditionDs = pageUI.getWidget(widgetId).getDataset("queryConditionDataset");
		else
			conditionDs = pageUI.getWidget(widgetId).getDataset("queryConditionTreeDataset");
	...
	imageResetMouseListener.onclick = function(MouseEvent) {
		...
		if (pageUI.getTab('sqlTab').getSelectedIndex() == 0) {
			...
			conditionDs = pageUI.getWidget(widgetId).getDataset("queryConditionTreeDataset");
			// 删除已显示内容
			var advanceTree = pageUI.getWidget(widgetId).getComponent("advanceTree");
			advanceTree.rootNode.clearChildren();
		}
		// 删除原有Dataset内容
		conditionDs.removeRowSet(conditionDs.currentKey);
		// 重新加载Dataset内容
		conditionDs.setCurrentPage(Dataset.MASTER_KEY, 0);
		//conditionDs.undo();
	};
	...
	/*收藏夹*/		
	var toolbar_2 = new ToolBarComp(document.getElementById("favoritDiv"), "savedConditionToolbar", 0, 0, "100%", "", "relative", null, false);
	toolbar_2.addButton("imageFolder", "", "生成文件夹", window.themeGlobalPath + "/themes/" + window.themeId + "/images/querytemplate/folder.png", "right", true);
	toolbar_2.addButton("imageDelete", "", "删除", window.themeGlobalPath + "/themes/" + window.themeId + "/images/querytemplate/delete.png", "right", true);
	//生成文件夹
...
function doConditionSave()
{
	...
	//var saveName = pageUI.getWidget(widgetId).getComponent("saveText").getValue();
...
function deleteFavorit()
{
	...
	var selectRow = saveDs.getSelectedRow();
	if(selectRow != null){
		var node = pageUI.getWidget(widgetId).getComponent("savedTree").getNodeByRowId(selectRow.rowId);
